...
#[structopt(name = "fetch_panels", about = "fetch panelapp data")]
struct Options {
...
async fn main() -> Result<(), Error> {
    ...
    for pan in &panel_genes {
        ...
        serde_json::to_writer(
            &File::create(format!("{}/{}.json", output, pan.id)).unwrap(),
            &pan,
        )
        .unwrap();
    }
...
async fn get_res(client: &Client, page: u8) -> Result<Top, Error> {
    let url: String = format!(
        "https://panelapp.genomicsengland.co.uk/api/v1/panels/?page={}",
        page
    );
